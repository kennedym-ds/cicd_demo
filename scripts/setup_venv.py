#!/usr/bin/env python3
"""
Virtual Environment Setup Helper for Python CI/CD Demo

This script helps set up a virtual environment for the Python CI/CD Demo project by:
...
class VenvSetup:
    ...
    def find_python_installations(self) -> List[PythonInstallation]:
        ...
        print(
            f"{Colors.OKBLUE}Searching in {len(unique_paths)} directories...{Colors.ENDC}"
        )
        ...
        for installation in sorted(
            installations, key=lambda x: x.version_tuple, reverse=True
        ):
        ...
        if unique_installations:
            self.print_colored(
                f"Found {len(unique_installations)} Python installation(s):",
                Colors.OKGREEN,
            )
    ...
    def select_python_version(
        ...
        if python_path:
            ...
            self.print_colored(
                f"Using specified Python: {installation}", Colors.OKGREEN
            )
            return installation
        ...
        if len(installations) == 1:
            ...
            self.print_colored(
                f"Using only available Python: {installation}", Colors.OKGREEN
            )
            return installation
        ...
        while True:
            try:
                ...
                if 0 <= choice_idx < len(installations):
                    ...
                    self.print_colored(
                        f"Invalid choice. Please enter 1-{len(installations)}",
                        Colors.WARNING,
                    )
    ...
    def create_virtual_environment(
        ...
        if venv_path.exists():
            if force:
                self.print_colored(
                    f"Removing existing virtual environment...", Colors.WARNING
                )
        ...
        self.print_colored(
            f"Creating virtual environment at: {venv_path}", Colors.OKBLUE
        )
        ...
        try:
            ...
            self.print_colored(
                "Virtual environment created successfully!", Colors.OKGREEN
            )
            return venv_path
    ...
    def install_dependencies(
        ...
        try:
            ...
            if upgrade_result.returncode == 0:
                ...
                self.print_colored(
                    "Continuing with existing pip version...", Colors.WARNING
                )

        except (subprocess.CalledProcessError, subprocess.TimeoutExpired) as e:
            self.print_colored(
                f"Warning: Could not check/upgrade pip: {e}", Colors.WARNING
            )
            self.print_colored(
                "Continuing with existing pip version...", Colors.WARNING
            )
        ...
        if pyproject_path.exists():
            ...
            if is_poetry_project and poetry_lock_path.exists():
                self.print_colored(
                    "Found Poetry project, checking for Poetry installation...",
                    Colors.OKBLUE,
                )
                ...
                try:
                    ...
                    if poetry_result.returncode == 0:
                        self.print_colored(
                            f"Using Poetry: {poetry_result.stdout.strip()}",
                            Colors.OKGREEN,
                        )
                        ...
                        # Add Poetry groups based on options
                        groups = []
                        if dev:
                            groups.append("dev")
                        # Poetry uses groups, not extras for dev dependencies
                        ...
                        try:
                            ...
                            self.print_colored(
                                f"Poetry install failed: {e}", Colors.WARNING
                            )
                            self.print_colored(
                                "Falling back to pip installation...", Colors.WARNING
                            )
                    ...
                    self.print_colored(
                        "Poetry not found, using pip installation...", Colors.WARNING
                    )

            # Fallback to pip installation for pyproject.toml
            self.print_colored(
                "Found pyproject.toml, installing project in development mode...",
                Colors.OKBLUE,
            )
            ...
            for req_file in requirements_files:
                ...
                if req_path.exists():
                    ...
                    try:
                        ...
                        self.print_colored(
                            f"Error installing from {req_file}: {e}", Colors.WARNING
                        )

            if not installed_any:
                self.print_colored(
                    "No requirements files found. Installing basic packages...",
                    Colors.WARNING,
                )
                ...
                try:
                    ...
                    self.print_colored(
                        f"Error installing basic packages: {e}", Colors.WARNING
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Set up virtual environment for Python CI/CD Demo project",
    ...
    parser.add_argument("--python", help="Use specific Python executable path")

    parser.add_argument(
        "--force", action="store_true", help="Force recreate if venv already exists"
    )
    ...
    try:
        ...
        venv_setup.print_colored(
            f"Python CI/CD Demo Virtual Environment Setup", Colors.HEADER + Colors.BOLD
        )
        ...
        venv_path = venv_setup.create_virtual_environment(
            python_installation, args.venv_name, args.force
        )
